...
const DigestsClient: React.FC = () => {
  const { theme } = useTheme();
  const [selectedCategory, setSelectedCategory] = useState<string>('all');
  const { magazines: digests, loading, error, refetch } = useMagazinesByType('digest');
  useScrollToTop();

  // Get unique categories from digests
  const categories = ['all', ...Array.from(new Set(digests.map(digest => digest.category)))];

  const filteredDigests = selectedCategory === 'all' 
    ? digests 
    : digests.filter(digest => digest.category === selectedCategory);
  ...
  if (loading) {
    return (
      <div className={`min-h-screen transition-colors duration-300 ${
        ...
        <LoadingSpinner text="Loading digests..." />
      </div>
    );
  }
        ...
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16">
          <div className="mb-8">
            <h1 className={`text-2xl font-bold mb-2 transition-colors duration-300 ${
              ...
              {selectedCategory === 'all' ? 'All Digests' : `${selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1)} Digests`}
            </h1>
            <p className={`transition-colors duration-300 ${
              theme === 'dark' ? 'text-primary-text' : 'text-light-primary-text'
            }`}>
              {filteredDigests.length} digest{filteredDigests.length !== 1 ? 'es' : ''} found
            </p>
          </div>
          ...
          {filteredDigests.length > 0 ? (
            ...
            <div className={`text-center py-12 transition-colors duration-300 ${
              ...
              <h2 className="text-xl font-semibold mb-2">No digests found</h2>
      ...
      <section className={`py-16 transition-colors duration-300 ${
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className={`text-lg mb-8 transition-colors duration-300 ${
            ...
            Receive curated summaries of the best content every week
          </p>
